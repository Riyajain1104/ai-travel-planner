...
def retry_if_retryable_exception(
    retry_exceptions: tuple,
):
    """Build a retry predicate that excludes API 429 responses."""

    from tenacity import retry_if_exception

    def is_retryable(exception: BaseException) -> bool:
        if not isinstance(exception, retry_exceptions):
            return False
        if isinstance(exception, APIError) and exception.status_code == 429:
            return False
        return True

    return retry_if_exception(is_retryable)
...
def with_retry(
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            @retry(
                retry=retry_if_retryable_exception(retry_exceptions),
