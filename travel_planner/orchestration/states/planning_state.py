...
class TravelPlanningState(BaseModel):
    ...
    travel_checkpoint_id: str | None = None
    ...
    # Individual parallel search results
    flight_search_result: Any = None
    accommodation_search_result: Any = None
    transportation_search_result: Any = None    # Human feedback and guidance
    ...
    def mark_interrupted(self, reason: str) -> None:
        ...
        self.travel_checkpoint_id = f"checkpoint_{uuid.uuid4().hex}"
    ...
    def create_checkpoint(self) -> dict[str, Any]:
        ...
        """
        checkpoint_data = self.model_dump(mode="json")

        # Generate a unique checkpoint ID if not already set
        if not self.travel_checkpoint_id:
            self.travel_checkpoint_id = f"checkpoint_{uuid.uuid4().hex}"

        checkpoint_data["checkpoint_id"] = self.travel_checkpoint_id
