...
def display_travel_plan(plan: TravelPlan) -> None:
    """Display a travel plan using the current TravelPlan schema."""
    ...
    if plan.flights:
        print("Flights:")
        for i, flight in enumerate(plan.flights, 1):
            departure = flight.departure_time
            arrival = flight.arrival_time
            dep_text = departure.strftime("%Y-%m-%d %H:%M") if hasattr(departure, "strftime") else str(departure)
            arr_text = arrival.strftime("%Y-%m-%d %H:%M") if hasattr(arrival, "strftime") else str(arrival)
            print(f"  {i}. {flight.airline}: {flight.departure_location} to {flight.arrival_location}")
            print(f"     {dep_text} - {arr_text}")
            print(f"     Price: {flight.currency} {float(flight.price):.2f}")
        print()

    if plan.accommodation:
        print("Accommodations:")
        for i, acc in enumerate(plan.accommodation, 1):
            acc_type = getattr(acc.type, "value", acc.type)
            print(f"  {i}. {acc.name} ({acc_type})")
            print(f"     {acc.location} - {acc.address}")
            print(f"     Price: {acc.currency} {float(acc.price_per_night):.2f} per night (Total: {float(acc.total_price):.2f})")
        print()

    if plan.activities:
        print("Daily Itinerary:")
        if isinstance(plan.activities, dict):
            days = sorted(plan.activities.items(), key=lambda item: getattr(item[1], "day_number", 0))
        else:
            days = list(enumerate(plan.activities))

        for index, day in days:
            if isinstance(day, str):
                print(f"  Day {index}: {day}")
                continue
            date_value = getattr(day, "date", None)
            date_text = date_value.strftime("%Y-%m-%d") if hasattr(date_value, "strftime") else str(date_value or "Unknown")
            day_number = getattr(day, "day_number", index)
            print(f"  Day {day_number} ({date_text}):")
            for j, activity in enumerate(getattr(day, "activities", []), 1):
                name = getattr(activity, "name", str(activity))
                ...
                if description:
                    print(f"        {description}")
                if cost is not None:
                    print(f"        Price: {getattr(activity, 'currency', 'INR')} {float(cost):.2f}")
            print()

    if plan.budget:
        print("Budget Summary:")
        print(f"  Total Budget: {plan.budget.currency} {float(plan.budget.total_budget):.2f}")
        print(f"  Spent: {plan.budget.currency} {float(plan.budget.spent):.2f}")
        print(f"  Remaining: {plan.budget.currency} {float(plan.budget.remaining):.2f}")
        if plan.budget.breakdown:
            print("  Breakdown:")
            for category, amount in plan.budget.breakdown.items():
                category_name = getattr(category, "value", category)
                print(f"     {category_name}: {plan.budget.currency} {float(amount):.2f}")
        print()
...
def _write_text_day_weather(f: TextIO, day: Any) -> None:
...
def _write_text_day_activities(f: TextIO, day: Any) -> None:
...
def _write_text_day_transportation(f: TextIO, day: Any) -> None:
...
def _generate_html_day_card(day: Any) -> str:
...
def _generate_html_day_activities(day: Any) -> str:
...
def _generate_html_day_transportation(day: Any) -> str:
...
async def run_query_mode(args: argparse.Namespace) -> TravelPlanningState:
    ...
    if args.save_to and final_state and final_state.plan:
        await save_travel_plan(final_state.plan, args.save_to, args.format)
        logger.info(f"Travel plan saved to {args.save_to}")

    # Save to database if requested
    if args.save_to_db and final_state and final_state.plan:
        await save_to_database(final_state.plan)
...
def build_travel_query_from_args(args: argparse.Namespace) -> TravelQuery:
    ...
    if args.budget:
        try:
            # Accept either a range ("10000-15000") or a single total
            # budget ("15000").
            budget_text = str(args.budget).replace(",", "").strip()
            if "-" in budget_text:
                min_val, max_val = map(float, budget_text.split("-", 1))
            else:
                min_val = max_val = float(budget_text)

            if min_val < 0 or max_val < min_val:
                raise ValueError("budget must be non-negative and ordered")

            query_params["budget_range"] = {
                "min": min_val,
                "max": max_val,
            }
        except ValueError:
            logger.warning(
                ...
                f"Use a single amount like '15000' or a range like '10000-15000'."
            )
...
def create_travel_workflow(args: argparse.Namespace) -> TravelWorkflow:
    """Create the workflow using the current registry-based architecture.

    Agent construction is intentionally delegated to ``TravelWorkflow``.
    The workflow registers the current agent implementations through the
    central agent registry, so the CLI does not pass constructor arguments
    from older versions of the agents.

    Args:
        args: Command-line arguments. Kept for CLI/API compatibility.

    Returns:
        Configured TravelWorkflow.
    """
    # The current TravelWorkflow owns agent construction and registration.
    # Do not instantiate agents here: their constructors no longer accept the
    # legacy browser_options/research_tools/workflow-agent keyword arguments.
...
async def _run_query_mode_with_display(args: argparse.Namespace) -> int:
    ...
    # Display the results
    if final_state and final_state.plan:
        display_travel_plan(final_state.plan)
        return 0
