...
class BudgetContext(AgentContext):
    ...
    allocations: dict[ExpenseCategory, BudgetAllocation] = Field(
        default_factory=dict
    )
    expenses: list[BudgetItem] = Field(
        default_factory=list
    )
    recommendations: list[BudgetRecommendation] = Field(
        default_factory=list
    )
    currency_conversions: dict[tuple[str, str], float] = Field(
        default_factory=dict
    )
    category_preferences: dict[ExpenseCategory, int] = Field(
        default_factory=dict
    )
...
class BudgetManagementAgent(BaseAgent[BudgetContext]):
    ...
    async def _extract_budget_parameters(
        ...
        # Read the structured travel query produced by query analysis.
        # The previous implementation left these fields at their defaults,
        # which made every budget summary show 0 USD.
        try:
            import json
            ...
            travel_query = payload.get("travel_query", {}) or {}
        except (TypeError, ValueError, json.JSONDecodeError):
            travel_query = {}

        budget_range = travel_query.get("budget_range") or {}
        if isinstance(budget_range, dict):
            minimum = budget_range.get("min")
            maximum = budget_range.get("max")
            if maximum is not None:
                context.total_budget = float(maximum)
            elif minimum is not None:
                context.total_budget = float(minimum)

        requirements = travel_query.get("requirements") or {}
        if isinstance(requirements, dict):
            context.currency = str(requirements.get("currency") or context.currency)

        context.traveler_count = int(travel_query.get("travelers") or context.traveler_count)

        start_date = travel_query.get("departure_date")
        end_date = travel_query.get("return_date")
        if start_date and end_date:
            try:
                from datetime import date
                start = date.fromisoformat(str(start_date))
                end = date.fromisoformat(str(end_date))
                context.trip_duration_days = max((end - start).days + 1, 1)
            except ValueError:
                context.trip_duration_days = 0
