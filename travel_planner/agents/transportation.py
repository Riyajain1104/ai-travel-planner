...
class TransportationContext(AgentContext):
    ...
    selected_options: dict[str, TransportationOption] = Field(
        default_factory=dict
    )
...
class TransportationAgent(BaseAgent[TransportationContext]):
    ...
    def __init__(self, config: AgentConfig | None = None):
        ...
        default_config = AgentConfig(
            ...
            Your goal is to research, compare, and recommend the best
            transportation options for travelers at their destination.
            Consider factors like convenience, cost, travel time,
    ...
    async def process(
        ...
        if not context.search_params:
            await self._extract_transportation_requirements(
                input_data,
                context,
            )

        airport_transfers = await self._plan_airport_transfers(
            context
        )

        context.transportation_options["airport_transfers"] = (
            airport_transfers
        )

        local_transportation = await self._research_local_transportation(
            context
        )

        context.transportation_options["local"] = (
            local_transportation
        )

        if context.points_of_interest:
            poi_transportation = await self._plan_poi_transportation(
                context
            )

            context.transportation_options["points_of_interest"] = (
                poi_transportation
            )

        plan_summary = await self._generate_transportation_plan(
            context
        )
    ...
    async def _extract_transportation_requirements(
        ...
        extraction_prompt = (
            "Please extract transportation planning requirements from "
            "the following user input. Include destination, accommodation "
            "location, preferred transportation types, traveler count, "
            "whether there are children or accessibility needs, and any "
            "budget constraints. Format your response as a JSON object.\n\n"
            "User input: {input}"
        )
            ...
            messages.append(
                {
                    ...
                    "content": (
                        ...
                        f"{context.search_params}"
                    ),
                }
            )

        # The original implementation uses demonstration values here.
        context.destination = "Paris, France"
        context.accommodation_location = (
        ...
        context.search_params = {
            ...
            "preferred_transportation_types": [
                ...
                for transport_type in context.preferred_transportation_types
            ],
    ...
    async def _plan_airport_transfers(
        ...
        mock_transfers = [
            TransportationOption(
                ...
                details=(
                    "Shared shuttle service, departs every 30 minutes"
                ),
            ...
            TransportationOption(
                ...
                start_location="Charles de Gaulle Airport",
                end_location=(
                    "Near " + context.accommodation_location
                ),
                provider="RATP",
                details=(
    ...
    async def _research_local_transportation(
        ...
        mock_local_options = [
            TransportationOption(
                ...
                provider="RATP",
                details=(
                    "Unlimited metro, bus, and RER within Paris"
                ),
                eco_friendly=True,
            ),
            TransportationOption(
                ...
                details=(
                    "Access to 20,000 bikes across 1,400 stations in Paris"
                ),
                eco_friendly=True,
            ),
            TransportationOption(
                ...
                provider="Europcar",
                details=(
                    "Renault Clio or similar, unlimited mileage"
                ),
    ...
    async def _generate_transportation_plan(
        ...
        plan_prompt = (
            "Create a detailed transportation plan for a trip to "
            "{destination} staying at {accommodation}. The trip is from "
            "{start_date} to {end_date} for {traveler_count} traveler(s).\n\n"
            "Please include:\n"
        ...
        if context.has_children:
            accessibility_note += (
                "Note: Traveling with children. "
            )

        if context.has_accessibility_needs:
            accessibility_note += (
                "Note: Has accessibility requirements. "
            )
