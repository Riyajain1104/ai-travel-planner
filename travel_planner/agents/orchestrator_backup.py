...
class OrchestratorAgent(BaseAgent[OrchestratorContext]):
    ...
    async def _extract_requirements(
        ...
        if isinstance(input_data, str):
            ...
            # The workflow passes a serialized TravelQuery. Extract the original
            # natural-language request before running deterministic parsing.
            try:
                payload = json.loads(user_input)
                if isinstance(payload, dict) and payload.get("raw_query"):
